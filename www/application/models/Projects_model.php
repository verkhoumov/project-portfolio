	...
	public function get_query_info($query = NULL)
	{
		...
		if (!empty($query)) 
		{
			...
			$query = "
				SELECT * 
				FROM (	
					SELECT `id`, `code`, `name`, `title`, `description`, `text`, NULL as `link`, NULL as `image`, NULL as `color`, 'category' as `type` FROM `{$this->db_categories}`
					UNION
					SELECT `id`, `code`, `name`, `title`, `description`, `text`, `link`, `image`, `color`, 'tech' as `type` FROM `{$this->db_skills}`
					UNION
					SELECT `id`, `code`, `name`, `title`, `description`, `text`, NULL as `link`, NULL as `image`, NULL as `color`, 'tag' as `type` FROM `{$this->db_tags}`
				) AS `list`
